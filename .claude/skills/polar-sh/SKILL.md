...
description: |
  ...
  - Migrating from a legacy billing provider to Polar
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {products.proMonthly && (
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">${(products.proMonthly.prices[0].priceAmount ?? 0) / 100}/mo</p>
            <Button onClick={() => handleSubscribe('proMonthly')}>Subscribe</Button>
          </CardContent>
        </Card>
      )}
    </div>
...
export const createCustomer = mutation({
  ...
  handler: async (ctx, args) => {
    // Get current limits
    const subscription = await polar.getCurrentSubscription(ctx, {});
    const productKey = subscription?.status === 'active' ? subscription.productKey : 'free';
...
## Migrating from a legacy billing provider

### 1. Replace Dependencies

```bash
npm uninstall <legacy-billing-sdk>
npm install @convex-dev/polar
```
...
Remove legacy billing fields, add Polar fields:
...
export default defineSchema({
  orgs: defineTable({
    name: v.string(),
    // Remove: legacyBillingCustomerId, legacyBillingSubscriptionId
...
Replace legacy billing queries/mutations with Polar equivalents:
...
export const getBillingInfo = query({
  handler: async (ctx) => {
    ...
    // Query legacy billing API
    const subscription = await fetchLegacySubscription(org.subscriptionId);
...
<UpgradeButton planId="pro" email={org.email} orgName={org.name} orgConvexId={org._id} />;
