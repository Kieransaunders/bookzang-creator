...
export function LibraryPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "imported": return "bg-blue-500/20 text-blue-300 border-blue-500/30";
      case "cleaned": return "bg-yellow-500/20 text-yellow-300 border-yellow-500/30";
    ...
    <div className="space-y-6">
      ...
      {books.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate-400">Import your first book from Project Gutenberg or upload a text file to get started</p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full border ${getStatusColor(book.status)} ml-2 flex-shrink-0`}>
                    {book.status}
                  </span>
                </div>
                ...
                <div className="space-y-2 text-sm text-slate-300">
                  ...
                  <div className="flex items-center gap-2">
                    <Calendar size={14} />
                    <span>{new Date(book.importedAt).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            </div>
